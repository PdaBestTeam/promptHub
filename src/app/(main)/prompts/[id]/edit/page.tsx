...
export default function EditPromptPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (pr.error || pr.author?.id !== user?.id) { router.push(`/prompts/${id}`); return; }
      setTitle(pr.title);
  ...
  if (loading) return <div style={{ paddingTop: 80, textAlign: "center", color: "var(--text-muted)" }}>불러오는 중...</div>;
  ...
  return (
    <div style={{ paddingTop: 60 }}>
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 36px 80px" }}>
        ...
        <div style={{ background: "var(--accent-dim)", border: "1px solid var(--accent-border)", borderRadius: 12, padding: "14px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 18 }}>📋</span>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>현재 v{currentVersionNo} 수정 중</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>저장 시 v{currentVersionNo + 1}로 업데이트됩니다.</div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: ".7px", marginBottom: 6 }}>제목</label>
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, marginBottom: 16 }}>
            ...
            <textarea style={{ width: "100%", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, color: "var(--text)", fontSize: 13, fontFamily: "inherit", outline: "none", resize: "vertical", minHeight: 220, lineHeight: 1.8 } as React.CSSProperties}
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, marginBottom: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".9px", color: "var(--text-muted)", marginBottom: 18, paddingBottom: 10, borderBottom: "1px solid var(--border)" }}>
              버전 메모 <span style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(선택)</span>
            </div>
            <input style={inputStyle} placeholder="이번 버전에서 변경한 내용을 간략히 적어주세요." value={changeNote} onChange={(e) => setChangeNote(e.target.value)} />
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>버전 히스토리에 표시됩니다.</div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 8 }}>
            <button type="button" className="btn-secondary" onClick={() => router.push(`/prompts/${id}`)}>← 취소</button>
