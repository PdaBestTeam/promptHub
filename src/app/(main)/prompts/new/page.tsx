...
export default function NewPromptPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (fromForkId) {
      ...
      if (!res.ok) { setError(data.error ?? "저장에 실패했습니다."); setSaving(false); return; }
  ...
  const isForkDraft = !!fromForkId && nextVersionNoOnSave != null && sourceVersionNo != null;
    ...
    <div style={{ paddingTop: 60 }}>
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 36px 80px" }}>
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(24px,3vw,36px)", letterSpacing: "-1.2px", color: "var(--text)", marginBottom: 6 }}>
            {isForkDraft ? "Fork로 새 프롬프트 만들기" : "새 프롬프트 등록"}
          </h1>
          <p style={{ fontSize: 14, color: "var(--text-muted)" }}>
            {isForkDraft ? "저장하면 포크가 반영됩니다. 저장하지 않고 나가면 카운트되지 않습니다." : "검증된 프롬프트를 커뮤니티와 공유하세요."}
          </p>
        </div>
        ...
        {isForkDraft && (
          <div style={{ background: "var(--accent-dim)", border: "1px solid var(--accent-border)", borderRadius: 12, padding: "14px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>v{sourceVersionNo}를 기반으로 작성 중</div>
              <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>저장 시 v{nextVersionNoOnSave}로 포크가 반영됩니다.</div>
            </div>
          </div>
        )}
